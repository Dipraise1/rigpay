...
impl Rpc {
    ...
    /// Solana Pay verification: the quote's reference pubkey appears in the paying
    /// transaction's account keys, so getSignaturesForAddress on the reference finds
    /// it. Payment amount is checked via token-balance deltas — no ATA derivation.
    pub async fn find_payment(
        ...
        let sigs = self
            .call("getSignaturesForAddress", json!([reference, {"limit": 20}]))
            .await?;
...
/// A Solana Pay reference is just a pubkey that the wallet includes in the
/// transaction's account keys. We generate a keypair and keep only the public
/// half — the secret is dropped on the floor, on purpose.
...
mod tests {
    ...
    #[test]
    fn reference_is_valid_base58_pubkey() {
        let r = new_reference();
